...
const CostCalculator = () => {
  ...
  const [length, setLength] = useState(1); // Длина
  const [width, setWidth] = useState(1); // Ширина
  const [area, setArea] = useState(1); // Площадь
  ...
  const handleTypeSelection = (type) => {
    ...
    setLength(1); // Сброс длины
    setWidth(1); // Сброс ширины
  ...
  const handleDimensionChange = (type, value) => {
    if (value === "" || Number(value) > 0) {
      if (type === "length") {
        ...
        setArea((Number(value) || 0) * width); // Учитываем текущее значение ширины
      } else if (type === "width") {
        setWidth(Number(value) || 0);
        setArea(length * (Number(value) || 0)); // Учитываем текущее значение длины
      }
    }
  };
